...
output_dir = "./outputs_10par_256samp_spline/"
appended = ""
    ...
    with torch.no_grad():
        ...
        for data, label in dataset:
            with torch.no_grad():
                ...
                output = model.sample(nsamples, conditional=data.to(torch.float32).flatten(start_dim=1).repeat((nsamples, 1)))
...
if model_type == "RealNVP":
    model = glasflow.RealNVP(
        n_inputs=num_params,
        n_transforms=3,
        n_conditional_inputs=length,
        n_neurons=32,
...
optimiser = torch.optim.AdamW([p for p in model.parameters()], 1e-5, weight_decay=1e-5)
